...
<div class="container-fluid">
            ...
            <div class="col-lg-4">
                <nav class="breadcrumb-container" aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">{{ __('Edit Banners')}}</li>
                    </ol>
                </nav>
            </div>
        </div>
    ...
    <div class="row">
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header"><h3>Edit Banners</h3></div>
                <div class="card-body">
                    <form method="post" action="{!! route('admin.banner.update',$taxonomy->term_taxonomy_id) !!}">
                        @method('put')
                        @csrf
                        <div class="form-group row">
                            <label for="checklistname" class="col-sm-2 col-form-label">Title</label>
                            <div class="col-sm-10">
                                <input type="text" class="form-control" id="checklistname" name="post_title" value="{{ $taxonomy->term->name }}" placeholder="Title">
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="checklistname" class="col-sm-2 col-form-label">Status</label>
                            <div class="col-sm-10">
                                <input type="checkbox" class="js-small" name="status" value="1" {{ ($taxonomy->term->meta->status==1)?'checked':'' }}/>
                            </div>
                        </div>
                        <h4 class="sub-title">Item</h4>
                        @if($taxonomy->posts()->count()>0)
                        @foreach($taxonomy->posts as $index=>$banner_image)
                        <div class="form-group row slide">
                            <div class="col-sm-3">
                                <input type="hidden" name="banner_image[{{ $index }}][id]" value="{{ $banner_image->ID }}">
                                <input type="text" class="form-control" name="banner_image[{{ $index }}][title]" value="{{ $banner_image->post_title }}" placeholder="Title">
                            </div>
                            <div class="col-sm-3">
                                <input type="text" class="form-control" name="banner_image[{{ $index }}][link]" value="{{ $banner_image->link }}" placeholder="Link">
                            </div>
                            <div class="col-sm-2">
                                <a href="#" id="thumb-image{{ $index }}" data-toggle="image" class="img-thumbnail">
                                    <img class="img-fluid" src="{{ asset($banner_image->image) }}" alt="" title="" data-placeholder="{{ asset('img/portfolio-8.jpg') }}">
                                </a>
                                <input type="hidden" name="banner_image[{{ $index }}][image]" value="{{ $banner_image->_thumbnail_id }}" id="input-image{{ $index }}">
                                <input type="hidden" name="banner_image[{{ $index }}][image_id]" value="{{ $banner_image->ID }}" id="input-image{{ $index }}">
                                <input type="file" name="file" class="d-none">
                            </div>
                            <div class="col-sm-2">
                                <input type="text" class="form-control" name="banner_image[{{ $index }}][sort_order]" placeholder="Sort Order">
                            </div>
                            <div class="col-sm-2">
...
@push('script')
        ...
        <script>
            $(function(){
                ...
                //
                var count='{{ $taxonomy->posts->count() }}'||1;//$('.slide').length;
                ...
                $(document).on('click','.delete-banner_image',function(e){
                    ...
                    if(confirm("Are you sure you want to delete this?")){
                        ...
                        var slide_id=$(this).parents('.slide').find('input[name*="[image_id]"]').val();
                        if(!slide_id){
                            $($this).parents('.slide').remove();
                            return false;
                        }
                        $.ajax({
                            type:'post',
                            url:'{!! route("admin.banner.update",$taxonomy->term_taxonomy_id) !!}',
                            data:{_method:'put', _token:'{!! csrf_token() !!}',slide_id:slide_id,type:'slide',action:'delete'},
                            success:function(data){
                                if(data.success){
                                    $($this).parents('.slide').remove();
                                }
